...
HEIGHT_LIST = [
    3635157
]  # if empty amount of tokens for current height only will be returned
...
def get_balances_on_height(height):
        ...
        supply_bostrom_df = (
            pd.DataFrame.from_records(
                get_json_from_bash_query(
                    f"cyber query bank total --chain-id bostrom --node https://rpc.bostrom.cybernode.ai:443 -o json --height {height}"
                )["supply"]
            )
            .set_index("denom")
    ...
    supply_bostrom_df = get_supply()

    def get_pools(supply_bostrom_df):
        pools_df = (
            pd.DataFrame.from_records(
                get_json_from_bash_query(
                    f"cyber query liquidity pools  --chain-id bostrom --node https://rpc.bostrom.cybernode.ai:443 -o json --height {height}"
                )["pools"]
            )
        ...
        pools_df["balances"] = pools_df["reserve_account_address"].map(
            lambda address: get_json_from_bash_query(
                f"cyber query bank balances {address} --chain-id bostrom --node https://rpc.bostrom.cybernode.ai:443  -o json --height {height}"
            )["balances"]
        )
    ...
    def calculate_prices(pools_df):
        ...
        prices_df["price_in_h"] = (
            prices_df["coin_2_amount"] / prices_df["coin_1_amount"]
        )
    ...
    def get_delegations(address: str):
        delegations = pd.DataFrame.from_records(
            get_json_from_bash_query(
                f"cyber query staking delegations {address} --chain-id bostrom --node https://rpc.bostrom.cybernode.ai:443 -o json --height {height}"
    ...
    def get_rewards(address: str):
        rewards = get_json_from_bash_query(
            f"cyber query distribution rewards {address} --chain-id bostrom --node https://rpc.bostrom.cybernode.ai:443 -o json --height {height}"
        )["total"]
        rewards_all_df = pd.DataFrame.from_records(rewards)
        rewards_all_df["amount"] = (
            rewards_all_df["amount"].astype("float_").astype("int64")
        )
    ...
    rewards_all_df = pd.concat(
        [get_rewards(address) for address in ADDRESSES_DICT.keys()]
    )
    ...
    def get_investminted_tokens(address: str):
        js1 = get_json_from_bash_query(
            f"cyber query account {address} --chain-id bostrom --node https://rpc.bostrom.cybernode.ai:443 -o json --height {height}"
        )
